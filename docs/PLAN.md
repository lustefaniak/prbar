...
**Phases 0, 1, 2, 3, 4, and 6 are shipped end to end. Persistence + several UX rounds beyond the original plan are in too.** 218 tests passing including real-API integration tests for both `gh` and `claude` (the latter gated by a `/tmp/prbar-run-claude-tests` sentinel since it costs real money — `bin/test` skips it by default). 6 SwiftData entry types share one `store.sqlite`: `ActionLogEntry`, `ReviewStateEntry`, `RepoConfigEntry`, `InboxSnapshotEntry`, `DiffCacheEntry`, `FailureLogCacheEntry`.
