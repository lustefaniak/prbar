...
**Phases 0, 1, 2, 3, 4, and 6 are shipped end to end. Persistence + several UX rounds beyond the original plan are in too.** 221 tests passing including real-API integration tests for both `gh` and `claude` (the latter gated by a `/tmp/prbar-run-claude-tests` sentinel since it costs real money — `bin/test` skips it by default). 6 SwiftData entry types share one `store.sqlite`: `ActionLogEntry`, `ReviewStateEntry`, `RepoConfigEntry`, `InboxSnapshotEntry`, `DiffCacheEntry`, `FailureLogCacheEntry`. The repo is past MVP and on `main` branch protection now — non-trivial work goes through PRs (squash-merged, linear history, `build-test` CI gate); doc-only edits and emergency fixes can still push directly via admin override. License: MIT.
...
- **"Reset all data" lever** (Settings → Diagnostics): deletes `store.sqlite` + `-shm` / `-wal` sidecars, clears the app's UserDefaults persistent domain, then relaunches. Destructive alert confirmation. Relaunch uses a detached `/bin/sh -c 'while kill -0 PID; do sleep 0.2; done; open APP'` watchdog so the new instance doesn't trip `enforceSingleInstance` against the still-terminating old one — the shell re-parents to launchd when we exit, fires `open` only once the old PID is gone. Belt-and-suspenders for the in-memory fallback path on a corrupt store.
- **Stable `RepoConfig.id`**: every config carries a UUID matched against `RepoConfigEntry.id`. Editing repoGlobs no longer invalidates row identity (sidebar selection survives, no draft-bleed into the next selection), and reorders update only `orderIndex` instead of churning rows. `RepoConfig.default` is now a computed property (was `static let`) so each `var cfg = .default` site gets a fresh id rather than colliding on the static UUID. `loadFromContext` force-syncs `config.id ← row.id` so legacy payloads stabilize on first read.
- **Drag-to-reorder repo rules** in the Repositories sidebar — `.onMove` on the List section, persisted via `RepoConfigStore.setAll`. Order matters because glob-match precedence is first-match-wins; the section footer reminds you to put specific rules above broad ones.
- **Refresh-after-action**: `PRPoller.postReview` fires an immediate `refreshPR` plus a forced second pass after 1.2s to catch GitHub's GraphQL read-model lag behind the gh REST write. Auto-approve fire wires through `ReviewQueueWorker.onAutoApproved` to do the same thing — previously the row sat in "review requested" state until the next 60s poll.
- **`skipAIIfReviewedByOthers` defaults to true**. Most PRs that already carry an APPROVED / CHANGES_REQUESTED decision from a human reviewer don't need a fresh AI run; the row stays visible regardless. Existing user configs keep whatever they explicitly saved.
- **Sparkle auto-update** (`SPUStandardUpdaterController` in `AppDelegate`, EdDSA-signed appcast hosted on GitHub Pages at `https://lustefaniak.github.io/prbar/appcast.xml`). Right-click menu and About tab both expose "Check for Updates…". Signing tools live under `bin/sparkle-tools|keygen|sign|appcast-add`; the release workflow signs the DMG with `SPARKLE_ED_PRIVATE_KEY` (GitHub secret), pulls the existing appcast from `origin/gh-pages`, appends the new entry, and pushes the regenerated `appcast.xml` back to `gh-pages` on each tag. First-time setup: run `bin/sparkle-keygen` once, paste the public key into `project.yml`'s `INFOPLIST_KEY_SUPublicEDKey`, then `bin/sparkle-keygen --export` and stash the private key in the GH secret.
- **Multiline pattern editors** (Root patterns + Ignore PRs by title): editor text lives in local `@State` buffers, not a Binding round-trip through `[String]`. The previous round-trip filtered empty lines on every keystroke, which silently erased the trailing newline you'd just typed and made Return appear broken. Buffers seed on appear / on `config.id` change; the persisted array is re-derived via `parsePatterns` (newline- *or* comma-separated, trimmed, empties dropped).
- **Notification action buttons + delegate routing** (`NotificationActionRouter`, `NotificationCategoryID`, `NotificationActionID`): registers `merge_ready` / `reviews_ready` / `ci_alert` `UNNotificationCategory` instances at launch with `[Merge all] [Open]` / `[Open]` actions and acts as the `UNUserNotificationCenterDelegate`. Banner taps and action presses route to `PRPoller.mergePR` / `NSWorkspace.open`. `userInfo` carries every event's `prNodeId` + URL so "Merge all" can resolve the full coalesced batch, not just the first PR.
- **Persistent `(prNodeId, headSha)` notification dedup** (`ReadinessCoordinator.notifiedSHAs`, `UserDefaultsNotifiedSHAStore`): same PR + same head SHA across restarts no longer re-pings; a new commit (different SHA) re-arms so the user knows the AI triaged a freshly-pushed commit. Capped at 500 entries to bound growth.
- **Cache-hit settled-pulse** in `ReviewQueueWorker.enqueue`: cache hits at the current `headSha` still fire `onReviewSettled` so `ReadinessCoordinator` can flush. Without this, a relaunch with persisted reviews + active review-requests produced zero notifications even with permissions granted.
- **Newest-first review queue draining**: `ReviewQueueWorker.drainIfPossible` pops max-PR-number instead of FIFO so a stale older PR can't block fresh review requests behind it. Per-repo monotonic; cross-repo it's an arbitrary-but-stable tiebreaker.
- **Verdict pill obeys `approveIncludesComment`**: clicking the colored Approve pill at the top of the AI Review section was always posting the AI summary as the body, ignoring the user's "post empty body on approve" toggle. Now branches on `(action == .approve && !approveIncludesCommentDefault)`. Comment / Request-changes still post the summary — posting them empty would be useless.
...
6. **Notification action buttons live.** `NotificationActionRouter` registers the `merge_ready` / `reviews_ready` / `ci_alert` categories with `[Merge all] [Open]` / `[Open]` actions and acts as the `UNUserNotificationCenterDelegate`. Banner taps and action presses route to `PRPoller.mergePR` / `NSWorkspace.open`. Auto-approve undo is still popover-only — see §"Still to do" #4.
...
9. **Merge surface in `PRDetailView` for authored PRs.** The row-level green split button only appears once a PR is `isReadyToMerge`; the detail view (popover + standalone window) now carries a parallel "merge card" — the author-side analogue of the review `actionsCard` — that is *always* shown for `role == .authored`/`.both`. It surfaces a merge-readiness status line (`InboxPR.mergeBlockReason`: draft / changes-requested / review-required / conflicts / checks pending|failing / branch-protection), the green squash/merge/rebase split button when ready, and an **Enable auto-merge** split button (`gh pr merge --auto`) when blocked but the repo allows it — plus **Disable auto-merge** when one is queued. New `ActionQueue` kinds `.enableAutoMerge(method:)` / `.disableAutoMerge` route through the same serialized, dedup'd, logged write path as `.merge`; `GHClient.mergePR` gained an `auto:` flag and a sibling `disableAutoMerge`. History logs them as `auto_merge_enable` / `auto_merge_disable`. Closes Goal #1's "one-click merge" for the detail surface and lands the auto-merge plumbing PLAN listed under §"AI-CLI commands" (`gh pr merge … --auto`).
...
- ✅ **Action buttons** (`[Merge all] [Open]` / `[Open]`): categories registered + routed via `NotificationActionRouter` (UNUserNotificationCenterDelegate). Default tap = Open primary URL. Merge all walks `userInfo.prNodeIds`, looks each up in `PRPoller.prs`, and fires `mergePR` with the repo's preferred allowed method (squash → merge → rebase). Auto-approve `[Undo]` still pending — banner itself isn't a UN notification yet.
