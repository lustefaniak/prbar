...
**Cutting a release = push a `v*.*.*` tag on HEAD.** Version is fully tag-derived (`bin/version` reads the latest tag); no `project.yml`/plist edit needed. The notes are the changelog — there is no `CHANGELOG.md`. The workflow's `generate-notes` step only fills the body when it's **empty**; a non-empty body is **preserved**. So for real release notes either pre-create a draft with a hand-written body *before* tagging, or rewrite after with `gh release edit <tag> --notes-file <f>` (the workflow won't clobber it). House style: `## <headline>` + `### Added` / `### Changed` / `### Internal` sections + the existing signed/notarized footer line (see v0.5.0 for the template). **Mechanics of the pre-create path:** `gh release create --draft` does NOT create/push the git tag (it shows as `untagged-…`), so it never triggers `release.yml`. Order it: `git tag vX.Y.Z <sha> && git push origin vX.Y.Z` (fires the build), *then* `gh release create vX.Y.Z --draft --notes-file …` against the now-existing tag. The publish step preserves your non-empty body — but its preserve path does **not** append the signed/notarized footer (only the auto-generate path does), so paste the footer into your hand-written notes yourself.
...
1. **`PRPoller.live()`** — 60s polling loop calling `GHClient.fetchInbox()`. Reads only: poll + per-PR `refreshPR` (single-PR query). `SnapshotCache` (actor) persists `[InboxPR]` so launches show known state immediately. After each poll, fires `EventDeriver.events(from:oldPRs:)` for *author-side* events (ready-to-merge, CI failed) and hands the full inbox to `ReadinessCoordinator.track(prs:)` — which now owns the review-request notification path. **All gh writes (post review, merge) moved out to `ActionQueue`** — the poller is read-only now.
1b. **`ActionQueue.live()`** — the single serialized path for gh *writes* (post review, merge, auto-approve). Mirrors `ReviewQueueWorker`'s drain pattern (`pending` / `inFlightNodes` / `drainIfPossible`). Per-PR single slot keyed by `nodeId` (a second enqueue while one is queued/running is a no-op — the accidental-double-submit guard); per-PR serialization with `maxConcurrent=2` across different PRs. Each write is captured as a `GHAction` value so a failure is retained as `.failed` and `retry()` re-runs it verbatim. Success removes the slot and fires `onActionCompleted` (wired in `AppDelegate` to the refresh-now + 1.2s-delayed-forced-refresh pair). Logs every attempt to `ActionLogStore`.
...
5. **`NotificationActionRouter`** (`@MainActor` NSObject) — `UNUserNotificationCenterDelegate`. Registers categories with `[Merge all] [Open]` / `[Open]` actions at launch and routes taps to `ActionQueue.enqueue(.merge)` / `NSWorkspace.open`. AppDelegate holds a strong reference (the system keeps the delegate weakly).
...
- **`GHClient`** (actor) wraps `gh`: `fetchInbox` / `fetchPR` / `fetchDiff` / `mergePR` / `postReview` / `postReviewWithComments`. Reads are called from `PRPoller` / `ReviewQueueWorker`; writes only from `ActionQueue`'s injected executors.
...
AI review pipeline:
- **`ClaudeProvider`** (struct, `ReviewProvider`) — spawns `claude -p --output-format stream-json --verbose --json-schema {…}`. Three modes: **`.sandboxed`** (the default) — the diff is **not** inlined; the AI explores a real worktree (provisioned at the PR head by `RepoCheckoutManager`) with `git diff <baseSha> HEAD`/grep/read. Broad `Bash` is allowed because `--settings '{"sandbox":{"enabled":true,…}}'` puts claude in an OS Seatbelt sandbox (read-only outside the disposable worktree, no network); `ContextAssembler` emits a lean prompt (PR meta + changed-file list + git instructions) instead of the diff. `.minimal` (Read/Glob/Grep + WebFetch/WebSearch, diff still inlined) and `.none` (pure-prompt, everything in `--disallowedTools`, empty temp cwd) remain as the inline-diff fallbacks. `.sandboxed` falls back to `.none` only when no checkout can be provisioned.
- **`CodexProvider`** (struct, `ReviewProvider`) — spawns `codex exec --skip-git-repo-check --output-schema <file> --output-last-message <file> --cd <workdir> --sandbox read-only -` (prompt on stdin). It already runs read-only + cwd-scoped, so `.sandboxed` needs **no argv change**: with a worktree workdir and the lean (diff-less) prompt, codex explores the change with `git diff`/grep inside its own read-only sandbox — the codex analogue of claude's Seatbelt path. Verified headless via a `codex exec --sandbox read-only` spike. Schema goes through `addStrictAdditionalProperties` (codex strict mode *requires* `additionalProperties:false`, the opposite of claude).
...
- `PRPoller(fetcher:, prRefresher:, cache:)` — inject closures. (Reads only: poll + single-PR refresh. All gh *writes* moved to `ActionQueue`.)
- `ActionQueue()` — set `reviewExecutor` / `mergeExecutor` closures (default no-op) and `onActionCompleted`; assert on `state(for:)`. See `ActionQueueTests`.
...
When you add a field to `InboxPR` / `RepoConfig`, declare it as `var x: T = <default>`, not `let` — a defaulted stored property keeps the synthesized memberwise init's argument optional, so the ~dozen `makePR` helpers compile unchanged. A `let` with no default forces threading it through every call site (grep `func makePR` / `InboxPR(` under `Tests/`; the compiler tells you which). Either way, add it to `CodingKeys` + `init(from:)` with `decodeIfPresent`.
...
- **`ActionQueue` is the only gh-write path.** Don't add ad-hoc `Task { await GHClient().postReview/mergePR }` at a call site — route through `actionQueue.enqueue(...)` so the write is serialized, dedup'd per PR, retryable, and logged once. The per-PR dedup is keyed on `pr.nodeId` (one in-flight write per PR); a second enqueue while busy is a silent no-op (the double-submit guard). Auto-approve goes through it too via `ReviewQueueWorker.enqueueAutoApprove` (the `approvePoster` closure is a tests-only fallback when that hook is nil). It's an env-injected `@Observable` service ⇒ adding/keeping it means updating `AppDelegate` (×2 env chains) + `PRBarApp` (×3 env chains) or the type is invisible at runtime and views crash on `@Environment(ActionQueue.self)`.
- **Inspect a stored AI review** without the UI: `sqlite3 "$HOME/Library/Application Support/io.synq.prbar/store.sqlite" "SELECT ZPAYLOAD FROM ZREVIEWLOGENTRY WHERE ZPRNUMBER=<n> ORDER BY Z_PK DESC LIMIT 1;"` → JSON with `verdict` / `summaryMarkdown` / `toolNamesUsed` / `annotations` (the AI-output keys are snake_case: `line_start`, `line_end`). `ZREVIEWSTATEENTRY.ZPAYLOAD` holds the live per-PR review state. Fastest way to see what a review actually produced when its output looks "strange".
- **A near-empty Time Profiler trace ≠ idle.** `xctrace record --template "Time Profiler" --attach <pid>` only samples *on-CPU* threads, so a main thread blocked off-CPU (I/O, a lock, AppKit NSControl re-hosting) yields ~0 samples and looks idle. Use `/usr/bin/sample <pid> <seconds> -f <out>` instead — it dumps every thread's stack regardless of state; read the "Sort by top of stack" section for the real hotspots.
- **Coordinating a live capture with the tester.** A backgrounded `sample` / `log stream` starts the instant it launches, but your "do X now" text only reaches the user when the turn ends — so the window misses the action. Instead: have the user *start* the interaction (scroll/click) and send any message *while it's ongoing*, then run a short foreground capture against the running pid.
- **Spike AI-CLI sandbox/permission behavior before wiring it.** When changing the `claude`/`codex` invocation (sandbox `--settings`, `--sandbox`, allowed tools, `--permission-mode`), verify headless against a 2-commit throwaway git repo first (`claude -p … --settings '{"sandbox":{"enabled":true,…}}' --allowedTools Bash …` / `codex exec --json --sandbox read-only --cd <repo> -`). Confirms tools run without prompts and writes/network are blocked for ~$0.05 before touching app code.
- **`.textSelection(.enabled)` on macOS hosts each text span in an NSTextField**, which SwiftUI re-adds + re-bezel-configures on every scroll tick inside a `ScrollView` — janky scroll wherever it's applied to dense content (e.g. rendered markdown with many spans). `MarkdownText` deliberately omits it. Diagnose via `sample`: `-[NSScroller trackKnob:]` → `_AppKitAddSubview` → `NSTextFieldCell _bezelConfiguration`.
- **New fields on `InboxResponse.PullRequestNode` must be optional** (`let x: String?`), or the raw-JSON fixtures in `InboxResponseTests` fail to decode with `keyNotFound`. Map with `?? <default>` in `InboxPR(node:)`. The live GraphQL always returns the field; optional only protects the fixtures.
- **`PopoverView.selectedPR` is a `@State` snapshot, not live.** It doesn't auto-track `poller.prs`, so the open detail view won't reflect post-action state changes (e.g. open→merged after a merge) unless you re-sync it in `.onChange(of: poller.prs)`. The standalone `PRDetailWindowView` already resolves the PR live from `poller.prs`.
- **A running app flushes `UserDefaults` on terminate.** `defaults delete <domain> <key>` while the app is alive gets clobbered when `bin/run` kills the old instance on relaunch. Kill first (`pkill -x PRBar`), then delete, then relaunch. App bundle id is `dev.lustefaniak.prbar`.
