...
AI review pipeline:
- **`ClaudeProvider`** (struct, `ReviewProvider`) — spawns `claude -p --output-format stream-json --verbose --json-schema {…}`. Three modes: **`.sandboxed`** (the default; claude only) — the diff is **not** inlined; the AI explores a real worktree (provisioned at the PR head by `RepoCheckoutManager`) with `git diff <baseSha> HEAD`/grep/read. Broad `Bash` is allowed because `--settings '{"sandbox":{"enabled":true,…}}'` puts claude in an OS Seatbelt sandbox (read-only outside the disposable worktree, no network); `ContextAssembler` emits a lean prompt (PR meta + changed-file list + git instructions) instead of the diff. `.minimal` (Read/Glob/Grep + WebFetch/WebSearch, diff still inlined) and `.none` (pure-prompt, everything in `--disallowedTools`, empty temp cwd) remain as the inline-diff fallbacks. `.sandboxed` falls back to `.none` when no checkout can be provisioned or for non-claude providers (codex never gets it).
